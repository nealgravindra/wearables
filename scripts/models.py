...
class Inception(nn.Module):
    def __init__(self, in_channels, bottleneck=32, kernel_size=60, nb_filters=64):
        ...
        '''
        super().__init__()
        self.bottleneck = nn.Conv1d(in_channels, bottleneck, 1, padding='same') # (bug) this is not really a bottleneck layer, REMOVE following: if bottleneck and in_channels > 1 else lambda x: x
        conv_layers = []
        kss = [kernel_size // (2**i) for i in range(3)] # (enhancement) 360 down to 5 min, for a total of 9 blocks?
        for i in range(len(kss)):
            conv_layers.append(
                nn.Conv1d(bottleneck, nb_filters, kernel_size=kss[i], padding='same')
                )
...
class InceptionBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size,
                 nb_filters=64, residual=True, depth=6):
        super().__init__()
