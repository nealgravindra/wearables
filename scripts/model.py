...
class GRU(nn.Module):
    ...
    def __init__(self, d_in, d_hidden, nb_layers, T, d_out):
        ...
        self.gru = nn.GRU(d_in, d_hidden, 
                            ...
                            dropout=0.5)
        # pred layer
        self.fc1 = nn.Linear(d_in*d_hidden, d_in*d_hidden // 2)
    ...
    def forward(self, X, addl_out):
        ...
        h_n = h_n.reshape(self.nb_layers, 2, -1, self.d_hidden)[-1]
        h_n = h_n.reshape(-1, 2*self.d_hidden)
        h_n = self.drop1(self.act1(self.fc1(h_n)))
        if addl_out:
            return self.fc2(h_n), h_n
        else:
            return self.fc2(h_n)
...
class CNN(nn.Module):
    ...
    def __init__(self, in_channels, L, d_out, conv_arch):
        ...
        self.fc1 = nn.Linear(out_channels*(L // (2**nb_halves)), 512)
        self.act1 = nn.LeakyReLU()
        self.drop1 = nn.Dropout()
